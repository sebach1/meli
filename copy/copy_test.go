...
func TestCopySameStructWithPointerField(t *testing.T) {
	...
	if user.Birthday == newUser.Birthday {
		t.Errorf("TestCopySameStructWithPointerField: Birthday failed since they need to have different address")
	}

	if user.FakeAge == newUser.FakeAge {
		t.Errorf("TestCopySameStructWithPointerField: FakeAge failed since they need to have different address")
	}
}
...
func TestCopyFromStructToSlice(t *testing.T) {
	...
	if Copy(&employees, &user); len(employees) != 1 {
		t.Errorf("Should only have one elem when struct to slice")
	} else {
...
func TestCopyFromSliceToSlice(t *testing.T) {
	...
	if Copy(&employees, users); len(employees) != 2 {
		t.Errorf("Should have two elems when slice to slice")
	} else {
